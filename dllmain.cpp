...
constexpr auto MODNAME = "000sound_mod"sv;
...
BOOL APIENTRY DllMain(HMODULE module, DWORD reasonForCall, LPVOID) {
    ...
    switch(reasonForCall) {
        case DLL_PROCESS_ATTACH: {
            ...
            modInstanceMutex = CreateMutexW(NULL, TRUE, L"SoundManager Instance");
            if (!modInstanceMutex) {
                break;
            }
          ...
          processStarted = CreateProcessW( 
                (rootDir / "mediaplayer/SoundManager.exe").native().c_str(),   // Executable path
